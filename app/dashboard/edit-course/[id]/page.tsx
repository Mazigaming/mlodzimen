    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.hasAccess) {
          ...
          setError('Nie masz uprawnień do edycji tego kursu');
        }
      } else {
    ...
    */
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.message || 'Błąd edycji kursu');
      }

      setSuccess(true);
      setTimeout(() => router.push('/dashboard'), 2000);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Błąd edycji kursu');
    } finally {
      setIsLoading(false);
    }
  }

  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white">Ładowanie kursu...</div>;

  if (!user || !course) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white">Brak dostępu</div>;
    ...
    <div className="min-h-screen py-32 px-4 bg-gradient-to-br from-slate-950/50 via-blue-950/25 to-slate-950/50 relative overflow-hidden">
      ...
      >
        <motion.div variants={itemVariants} className="flex justify-between items-end mb-12">
          <div>
            <h1 className="text-5xl md:text-7xl font-black text-white mb-4">Edytuj <span className="text-gradient-warm">Kurs</span></h1>
            <p className="text-xl text-gray-400 font-medium">Zaktualizuj swoją ścieżkę edukacyjną</p>
          </div>
          <Link href={`/kursy/${id}`} className="px-6 py-3 rounded-2xl bg-slate-900 border border-slate-700 text-gray-400 hover:text-white transition-all font-bold">Podgląd</Link>
        </motion.div>
              ...
              <div aria-live="polite">
                {success && (
                  <div className="bg-green-500/20 border border-green-500/50 text-green-200 px-6 py-4 rounded-2xl mb-6 font-bold flex items-center gap-3">
                    <span>🚀</span> Kurs zaktualizowany pomyślnie!
                  </div>
                )}
                {error && (
              ...
              <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select name="category" defaultValue={course.category} required className="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white text-sm focus:ring-2 focus:ring-blue-500 transition-all">
                  ...
                  <div>
                    ...
                    <select name="level" defaultValue={course.level} className="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white text-sm focus:ring-2 focus:ring-blue-500 transition-all">
                  ...
                  <div>
                    ...
                    <input type="number" name="price" step="0.01" min="0" defaultValue={course.price} placeholder="0.00" className="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white text-sm font-black focus:ring-2 focus:ring-blue-500 transition-all" />
                  </div>
                </div>
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="pl-6 space-y-2 border-l border-slate-800 ml-2">
                        {m.lessons.map((l: any, lIdx: number) => (
                          <div key={l.id || lIdx} className="flex flex-col gap-1 p-3 rounded-xl bg-slate-900/40 border border-slate-800/50 hover:border-slate-700 transition-all">
